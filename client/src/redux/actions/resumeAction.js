...
export const submitCV = (idJob, idCompany, cv, auth, socket) => async (dispatch) => {
    try {
        ...
        const res = await postDataAPI('submit-cv', { idJob, idCompany, idCV: cv._id, dataCV: cv, dateSubmit: dateFormat(date, 'mm/dd/yyyy') }, auth.token)
        console.log(res)
...
export const createResume = (data, avatar, token) => async (dispatch) => {
    try {
        ...
        if (avatar) media = await imageUpload([avatar])
            ...
            return dispatch({ type: GLOBALTYPES.ALERT, payload: { error: "Missing avatar" } })
        const res = await postDataAPI("create-resume",
            { ...data, avatar: media[0].url },
            token)
        dispatch({ type: GLOBALTYPES.ALERT, payload: { success: res.data.msg } })
    } catch (error) {
...
export const set_status = ({ idJob, idCV, status, idCandidate, auth, socket }) => async (dispatch) => {
    try {
        dispatch({ type: GLOBALTYPES.ALERT, payload: { loading: true } })
        const res = await postDataAPI("set_status", { idJob, idCV, status, idCandidate })
        dispatch({ type: GLOBALTYPES.ALERT, payload: { success: res.data.msg } })
        const msg = {
            id: res.data.idJob,
